...
export const TreeProvider = ({
  ...
  const currentExpandedIds = useMemo<Set<string>>(() => {
    return isExpandedControlled ? new Set(expandedIds) : internalExpandedIds;
  }, [isExpandedControlled, expandedIds, internalExpandedIds]);
  ...
  const handleSelection = useCallback(
    (nodeId: string, ctrlKey = false) => {
      ...
      if (multiSelect && ctrlKey) {
        next = currentSelectedIds.includes(nodeId)
          ? currentSelectedIds.filter((id) => id !== nodeId)
          : [...currentSelectedIds, nodeId];
      } else {
        next = currentSelectedIds.includes(nodeId) ? [] : [nodeId];
      }
...
export const TreeNodeTrigger = ({
  ...
  const { selectedIds, toggleExpanded, handleSelection, indent, multiSelect } =
    useTree();
...
export const TreeNodeCheckbox = () => {
  const { selectedIds, handleSelection, multiSelect } = useTree();
  const { nodeId } = useTreeNode();
  ...
  return (
    ...
    >
      <Checkbox
        checked={checked}
        onCheckedChange={() => handleSelection(nodeId, true)}
      />
    </div>
  );
};
