...
const EnhancedTreeItem: React.FC<{
  ...
  let iconClassName = "h-4 w-4 shrink-0 mr-2";
  ...
  return (
    <div className={`flex items-center py-2 ${paddingClass}`}>
      ...
      {item.isActive === false && (
        <CircleX className="h-3 w-3 ml-1 text-red-500" />
      )}
      {item.isActive === true && (
        <CircleCheck className="h-3 w-3 ml-1 text-green-500" />
      )}
    </div>
  );
};
  ...
  const convertToTreeData = (
    ...
    return departments.map((dep) => {
      ...
      const treeItem: EnhancedTreeItem = {
        ...
        onClick:
          dep.hasMoreChildren && (!dep.children || dep.children.length === 0)
            ? () => loadChildren(dep.id)
            : undefined,
      };
